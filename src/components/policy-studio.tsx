...
export function PolicyStudio({ templates, members, brand, canBrand }: { templates: Template[]; members: { id: string; name: string }[]; brand: Brand; canBrand: boolean }) {
  ...
  return <div className="space-y-6">
    <section className="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-950 to-emerald-700 p-6 text-white shadow-sm">
      ...
      <p className="mt-3 max-w-4xl leading-7 text-emerald-50">Build a professional policy suite around your organisation’s services, identity and responsibilities. Each policy combines practical operating guidance with its regulatory, employment-law and evidence sources, then follows the organisation’s ownership, review and approval process.</p>
    </section>

    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div><p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Licensed organisation identity</p><h2 className="mt-1 text-xl font-bold">Policy header branding</h2><p className="mt-1 text-sm text-slate-600">The name, logo, colour and contact details belong to this organisation account and appear in the policy header. The ATOM copyright and licence footer remains fixed.</p></div>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Policy collection</p><h2 className="mt-1 text-xl font-bold">Choose policies for your organisation</h2><p className="mt-1 text-sm text-slate-600">{templates.length} core adult social care policies. Existing active policies are recognised automatically and will not be duplicated.</p></div><div className="flex gap-2"><button type="button" onClick={() => setSelected(availableVisible.map(({ key }) => key))} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold">Select visible</button><button type="button" onClick={() => setSelected([])} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold">Clear</button></div></div>
