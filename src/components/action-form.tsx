...
export function ActionForm({ locations, owners, oversightOwners, clients, evidence, sources, initial, preselectedSource, prefill, riskHandoff }: { locations: Option[]; owners: Option[]; oversightOwners: Option[]; clients: Option[]; evidence: Option[]; sources: Source[]; initial?: Initial; preselectedSource?: string; prefill?: Values; riskHandoff?: { reference: string; residualScore: number; targetScore: number } }) {
  ...
  return <form onSubmit={submit} className="space-y-7">
    ...
    <Section number="3" title="Management response and progress" copy="A response records what management says has happened; it does not close the finding."><div className="grid gap-4 md:grid-cols-2">
      <Field label="Status"><select className={cls} name="status" defaultValue={value("status") ?? "OPEN"}>{ACTION_STATUSES.filter((item) => !["OVERDUE", "ARCHIVED", "COMPLETED"].includes(item)).map((item) => <option key={item} value={item}>{actionLabel(item)}</option>)}</select><span className="mt-1 block text-xs font-normal text-slate-500">Completion and closure are recorded through Progress and Assurance, not this editor.</span></Field>
    ...
    <Section number="4" title="Source Evidence and recurrence plan" copy="Link relevant source material here. Record completion Evidence with a progress update, then use Assurance for verification, effectiveness and closure."><div className="grid gap-4 md:grid-cols-2">
      <Field label="Source or supporting Evidence" wide><p className="mt-1 text-xs font-normal text-slate-500">Hold Ctrl or Command to choose more than one item. This does not prove completion.</p><select multiple className={`${cls} min-h-32`} name="evidenceIds" defaultValue={initial?.evidenceIds ?? []}>{evidence.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
