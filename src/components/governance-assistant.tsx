...
export function GovernanceAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      role: "assistant",
      text: "Hi, I’m Abi. I can explain every Hub module, why it matters in health and social care, how it may support CQC evidence, or take you to the right page. What would you like to know?",
    },
  ]);
  ...
  return <>
    {open ? <section role="dialog" aria-label="Chat with Abi" className="fixed bottom-4 right-4 z-50 flex h-[min(620px,calc(100vh-2rem))] w-[min(410px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-emerald-900/20 bg-white shadow-2xl">
      ...
      <p className="bg-white px-4 pb-3 text-[10px] leading-4 text-slate-500">Abi explains the Hub and general inspection-readiness concepts. She cannot give clinical, legal or regulatory advice, predict a CQC rating or replace current official guidance. This conversation is not saved.</p>
    </section> : null}
