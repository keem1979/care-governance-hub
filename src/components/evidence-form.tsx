...
export function EvidenceForm({ owners, locations, policies, initial, requirement }: { owners: Option[]; locations: Option[]; policies: PolicyOption[]; initial?: Initial; requirement?: EvidenceRequirement }) {
  ...
  return <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <label className="text-sm font-medium">Category<select className={field} name="category" defaultValue={initial?.category ?? requirement?.category ?? ""} required><option value="">Choose category</option>{EVIDENCE_CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select></label>
      <label className="text-sm font-medium">Evidence type<select className={field} name="evidenceType" defaultValue={initial?.evidenceType ?? (requirement ? "Record" : "")} required><option value="">Choose type</option>{EVIDENCE_TYPES.map((value) => <option key={value}>{value}</option>)}</select></label>
