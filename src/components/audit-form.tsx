...
export function AuditAssessmentForm({
  auditId,
  sections,
  saved,
  evidence,
  ...
  const update = (id: string, key: keyof ResponseValue, value: string) => setValues((current) => {
    const existing = current[id] ?? { answer: "", comment: "", evidenceId: "" };
  ...
  async function save(intent: "save" | "submit", form: HTMLFormElement) {
    ...
    const response = await fetch(`/api/audits/${auditId}/responses`, {
      ...
      body: JSON.stringify({
        ...
        responses: questions.map((question) => ({
          ...
          ...(values[question.id] ?? { answer: "", comment: "", evidenceId: "" }),
        })),
    ...
    if (!response.ok) {
      setError(result.error ?? "Could not save audit.");
      setBusy(false);
      return;
    }
    setBusy(false);
  ...
  if (!questions.length) {
    return <section id="audit-form" className="rounded-2xl border border-amber-200 bg-amber-50 p-6"><h2 className="text-xl font-bold">Audit form is not available</h2><p className="mt-2 text-sm">This template has no questions. Ask an administrator to publish a complete audit template before using it.</p></section>;
  }
  ...
  return <form id="audit-form" onSubmit={(event) => { event.preventDefault(); void save("save", event.currentTarget); }} className="space-y-6">
    <section className="rounded-2xl border border-emerald-200 bg-emerald-950 p-5 text-white shadow-sm">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-200">{readOnly ? "Completed record" : "Complete this audit form"}</p><h2 className="mt-1 text-2xl font-bold">{answered} of {questions.length} questions answered</h2><p className="mt-1 text-sm text-emerald-50/75">{readOnly ? "The form is read-only because this audit has progressed beyond assessment." : "Save at any time. Mandatory questions must be answered before review."}</p></div>
    ...
    {sections.map((section, sectionIndex) => <section key={section.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="divide-y divide-slate-200">
        {section.questions.map((question, index) => {
          const value = values[question.id] ?? { answer: "", comment: "", evidenceId: "" };
          return <fieldset key={question.id} className="py-6">
            <legend className="flex w-full gap-3">
              ...
              <span><span className="font-semibold">{question.text}{question.mandatory ? <span className="text-red-600"> *</span> : null}</span>{question.guidance ? <span className="mt-1 block text-sm font-normal text-slate-500">{question.guidance}</span> : null}<span className="mt-1 block text-xs font-normal text-slate-400">Evidence to check: {question.evidenceExpected ?? "Relevant supporting records"}</span></span>
            </legend>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6">
      <h2 className="text-xl font-bold">Audit conclusion</h2>
      <p className="mt-1 text-sm text-slate-600">Summarise the assurance gained, main risks and next steps.</p>
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        {[["strengths", "Strengths"], ["risks", "Risks and gaps"], ["recommendations", "Recommendations"]].map(([name, label]) => <label key={name} className="text-sm font-medium">{label}<textarea disabled={readOnly} name={name} defaultValue={summary[name as keyof typeof summary]} className="mt-1 min-h-24 w-full rounded-lg border border-slate-300 px-3 py-2 disabled:bg-slate-100" /></label>)}
        <label className="text-sm font-medium">Follow-up review date<input disabled={readOnly} name="reviewDate" type="date" defaultValue={summary.reviewDate} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 disabled:bg-slate-100" /></label>
      </div>
    </section>

    {!readOnly ? <div className="sticky bottom-4 flex flex-wrap gap-3 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-lg backdrop-blur">
      <button disabled={busy} className="rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold">{busy ? "Saving…" : "Save progress"}</button>
      <button disabled={busy} type="button" onClick={(event) => { if (event.currentTarget.form) void save("submit", event.currentTarget.form); }} className="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white">Submit completed form for review</button>
    </div> : null}
  </form>;
}
