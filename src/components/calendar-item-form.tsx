...
export function CalendarItemForm({ members, locations }: { members: Option[]; locations: Option[] }) {
  ...
  return <form onSubmit={submit} className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <FormPurpose title="Compliance deadline" description="Use this form for a renewal, review or submission deadline that is not already recorded elsewhere in QCGMS." steps={["Name the obligation", "Set the true due date and owner", "Explain the work needed before the deadline"]} />
