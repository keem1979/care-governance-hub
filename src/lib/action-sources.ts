...
export async function listActionSources(db:PrismaClient,context:{organisation:{id:string};allLocations:boolean;locations:{id:string}[]}):Promise<ActionSourceOption[]>{
  ...
  const[audits,registers,risks,policies,meetings]=await Promise.all([
    ...
    db.governanceMeeting.findMany({where:{organisationId:context.organisation.id,status:{not:"ARCHIVED"},...(context.allLocations?{}:{OR:[{locationId:null},{locationId:{in:context.locations.map(({id})=>id)}}]})},select:{id:true,reference:true,title:true},orderBy:{meetingDate:"desc"},take:100}),
  ]);
  return[
...
export async function resolveActionSource(db:PrismaClient,organisationId:string,type:string,id:string|null){
  ...
  if(type==="GOVERNANCE_MEETING"){const item=await db.governanceMeeting.findFirst({where:{id,organisationId},select:{reference:true,title:true,locationId:true}});if(item)return{reference:item.reference,title:item.title,locationId:item.locationId};}
