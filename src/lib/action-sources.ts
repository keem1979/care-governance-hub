...
export async function resolveActionSource(db: PrismaClient, context: SourceContext, type: string, id: string | null) {
  ...
  if (type === "AUDIT") { const item = await db.audit.findFirst({ where: { id, organisationId,...strict }, select: { title: true, locationId: true } }); if (item) return { reference: item.title, title: item.title, locationId: item.locationId, url: `/audits/${id}` }; }
  if (["COMPLAINT", "INCIDENT", "SAFEGUARDING", "REGISTER", "ASSESSMENT"].includes(type)) { const item = await db.registerEntry.findFirst({ where: { id, organisationId,...scoped }, select: { reference: true, title: true, locationId: true, definition: { select: { key: true } } } }); if (item) return { reference: item.reference, title: item.title, locationId: item.locationId, url: `/registers/${item.definition.key}/${id}` }; }
  if (type === "RISK") { const item = await db.risk.findFirst({ where: { id, organisationId,...scoped }, select: { reference: true, title: true, locationId: true } }); if (item) return { reference: item.reference, title: item.title, locationId: item.locationId, url: `/risks/${id}` }; }
  ...
  if (type === "GOVERNANCE_MEETING") { const item = await db.governanceMeeting.findFirst({ where: { id, organisationId,...scoped }, select: { reference: true, title: true, locationId: true } }); if (item) return { reference: item.reference, title: item.title, locationId: item.locationId, url: `/meetings/${id}` }; }
  if (type === "INSPECTION") { const item = await db.complianceRequirement.findFirst({ where: { id, organisationId,...scoped }, select: { title: true, keyQuestion: true, locationId: true } }); if (item) return { reference: `${item.keyQuestion} · ${item.title}`, title: item.title, locationId: item.locationId, url: `/inspection/${id}` }; }
  ...
  if (type === "EVIDENCE") { const item = await db.evidence.findFirst({ where: { id, organisationId, archivedAt: null,...scoped }, select: { title: true, locationId: true } }); if (item) return { reference: item.title, title: item.title, locationId: item.locationId, url: `/evidence/${id}` }; }
