...
describe("care quality overview", () => {
  ...
  it("flags active records and leaves closed records out of the queue", () => { expect(isQualityAttention("IN_REVIEW", "LOW")).toBe(true); expect(isQualityAttention("CLOSED", "HIGH")).toBe(false); expect(isQualityAttention("CLOSED", "LOW")).toBe(false); });
