...
export async function syncTrainingMatrixEvidence(tx: Prisma.TransactionClient, input: { organisationId: string; actorId: string }) {
  ...
  const data = { title: "Workforce training and competency matrix", description, category: "Training", evidenceType: "Record", ownerId: input.actorId, evidenceDate: new Date(), tags: ["system-generated", "workforce", "training-matrix", "regulation-18"], relatedModule: "WorkforceTrainingMatrix", relatedRecordId: "current", confidentiality: "RESTRICTED" as const, status: "ACTIVE" as const, notes: "Automatically refreshed from role assignments and dated staff training or competency records. Open Workforce → Training matrix for the live source." };
