...
export const ACTION_STATUSES = ["OPEN","IN_PROGRESS","AWAITING_EVIDENCE","AWAITING_VERIFICATION","COMPLETED","OVERDUE","CANCELLED","ARCHIVED"] as const;
export const ACTION_PRIORITIES = ["LOW","MEDIUM","HIGH","CRITICAL"] as const;
export const ACTION_SOURCE_TYPES = ["AUDIT","COMPLAINT","INCIDENT","SAFEGUARDING","RISK","GOVERNANCE_MEETING","POLICY_REVIEW","MANUAL"] as const;
...
export function sourcePath(type:string,id:string|null){if(!id)return null;if(type==="AUDIT")return`/audits/${id}`;if(["COMPLAINT","INCIDENT","SAFEGUARDING"].includes(type))return`/registers/${type.toLowerCase()}${type==="COMPLAINT"?"s":type==="INCIDENT"?"s":""}/${id}`;if(type==="RISK")return`/risks/${id}`;if(type==="POLICY_REVIEW")return`/policies/${id}`;return null}
