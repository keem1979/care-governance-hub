...
export default async function KpiReturnPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return <main className="mx-auto max-w-7xl space-y-6">
      ...
      <footer className="border-t pt-4 text-xs text-slate-500">QCGMS · Created by {item.createdBy.name}{item.submittedBy ? ` · Submitted by ${item.submittedBy.name}` : ""} · This internal record supports, but does not replace, the authority’s current submission process.</footer>
    </main>;
