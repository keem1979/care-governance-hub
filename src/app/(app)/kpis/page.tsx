    ...
    return <main className="space-y-7">
      ...
      <section className="overflow-hidden rounded-3xl bg-emerald-950 text-white shadow-sm">
        <div className="grid gap-5 p-6 lg:grid-cols-[1fr_auto] lg:items-center">
          ...
          <div className="flex flex-wrap gap-2">{canEdit ? <Link href="/kpis/monthly" className="rounded-xl bg-white px-5 py-3 text-sm font-bold text-emerald-950">Start this month’s return</Link> : null}<Link href="/kpis/returns" className="rounded-xl border border-white/30 px-5 py-3 text-sm font-bold text-white">View return history</Link></div>
        </div>
      </section>
      ...
      <section><div className="mb-4"><h2 className="text-xl font-bold">KPI scorecard</h2><p className="text-sm text-slate-600">{visible.length} standard indicators for {month}</p></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map(({ definition, entry }) => <article key={definition.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{definition.name}</h3><p className="mt-1 text-xs text-slate-500">{kpiLabel(definition.direction)} · {definition.unit}</p></div>{entry ? <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${ragClasses(entry.ragStatus)}`}>{entry.ragStatus}</span> : <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">AWAITING UPDATE</span>}</div>{entry ? <><p className="mt-5 text-3xl font-black">{entry.actualValue}<span className="ml-1 text-sm font-medium text-slate-500">{definition.unit}</span></p><div className="mt-3 grid grid-cols-3 gap-2 text-xs"><Metric label="Target" value={entry.targetValue} /><Metric label="Green" value={entry.greenThreshold} /><Metric label="Amber" value={entry.amberThreshold} /></div>{entry.notes ? <p className="mt-3 line-clamp-2 text-sm text-slate-600">{entry.notes}</p> : null}<p className="mt-3 text-xs text-slate-500">{entry._count.evidenceLinks} evidence link(s)</p></> : <p className="mt-5 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">Add this month’s result when it is ready.</p>}</article>)}</div></section>
