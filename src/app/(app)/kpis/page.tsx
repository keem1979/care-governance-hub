...
import { KpiScorecardEntry } from "@/components/kpi-scorecard-entry";
          ...
          {visible.map(({ definition, entry }) => {
            ...
            return <article key={definition.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                {entry ? <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${ragClasses(entry.ragStatus)}`}>{entry.ragStatus}</span> : <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">NEEDS ENTRY</span>}
              </div>
