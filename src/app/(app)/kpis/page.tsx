    ...
    return <main className="space-y-7">
      ...
      <section className="overflow-hidden rounded-3xl bg-emerald-950 text-white shadow-sm">
        <div className="grid gap-5 p-6 lg:grid-cols-[1fr_auto] lg:items-center">
          <div><h2 className="text-2xl font-bold">Monthly service performance return</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-emerald-50/80">Record branch delivery, workforce, complaints, safeguarding and referral activity once. The matching measures below will use the saved figures.</p></div>
      ...
      {canEdit ? <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
          <div><h2 className="text-lg font-bold text-emerald-950">Figures already held in QCGMS</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-emerald-900">The counts below come from registers, actions, workforce records, audits and policies. A figure entered and verified by a manager is kept unless the manager changes it.</p></div>
        ...
        {groupedVisible.map((section) => <section id={section.key} key={section.key} className="scroll-mt-24">
          ...
          {section.items.map(({ definition, entry }) => {
            ...
            return <article key={definition.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              {entry ? <><p className="mt-4 text-3xl font-black">{entry.actualValue}<span className="ml-1 text-sm font-medium text-slate-500">{definition.unit}</span></p><div className="mt-3 grid grid-cols-3 gap-2 text-xs"><Metric label="Target" value={entry.targetValue} /><Metric label="Green" value={entry.greenThreshold} /><Metric label="Amber" value={entry.amberThreshold} /></div><div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700"><span className="font-bold">Evidence source:</span> {kpiSourceLabel(entry.sourceType)}{entry.sourceUrl ? <> · <a href={entry.sourceUrl} target={entry.sourceUrl.startsWith("http") ? "_blank" : undefined} rel={entry.sourceUrl.startsWith("http") ? "noreferrer" : undefined} className="font-bold text-emerald-800 underline">Open source</a></> : null}</div>{entry.notes && !automaticallySynced ? <p className="mt-3 line-clamp-2 text-sm text-slate-600">{entry.notes}</p> : null}</> : <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">{calculated ? "Enter the related monthly totals and QCGMS will calculate this rate." : "No verified result has been recorded for this month."}</p>}
              {canEdit ? calculated ? <div className="mt-4 border-t border-slate-100 pt-4"><Link href={`/kpis/monthly?month=${month}${locationId ? `&location=${locationId}` : ""}`} className={`${KPI_RESULT_BUTTON_CLASS} w-full`}>Enter source figures</Link></div> : <KpiScorecardEntry definition={definition} month={month} locationId={locationId} actualValue={entry?.actualValue} notes={entry?.notes} sourceType={entry?.sourceType} sourceUrl={entry?.sourceUrl} /> : null}
            </article>;
          })}
          </div>
        </section>)}
