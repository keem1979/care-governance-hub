...
export default async function EvidencePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-widest text-emerald-700">Document repository</p><h1 className="text-3xl font-bold">Evidence Library</h1><p className="mt-1 text-slate-600">Inspection-ready evidence, expiry dates and controlled versions.</p></div><div className="flex flex-wrap gap-2"><Link href="/evidence/requirements" className="rounded-xl border border-emerald-700 bg-white px-5 py-3 text-sm font-semibold text-emerald-800">Required evidence and gaps</Link>{canUpload && <Link href="/evidence/new" className="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white">Upload evidence</Link>}</div></div>
...
function EvidenceCard({ item }: { item: { id:string; title:string; category:string; evidenceType:string; confidentiality:string; status:string; reviewExpiryDate:Date|null; owner:{name:string}; location:{name:string}|null; currentVersion:{versionNumber:string;fileName:string}|null; tags:string[] } }) {
  const display = evidenceDisplayStatus(item.status,item.reviewExpiryDate);
  return <Link href={`/evidence/${item.id}`} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-400"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">{item.category} · {item.evidenceType}</p><h2 className="mt-1 text-lg font-bold">{item.title}</h2><p className="mt-1 text-xs text-slate-500">{item.currentVersion?.fileName}</p></div><Badge value={display} /></div><dl className="mt-5 grid grid-cols-3 gap-3 text-sm"><div><dt className="text-slate-500">Owner</dt><dd className="font-medium">{item.owner.name}</dd></div><div><dt className="text-slate-500">Location</dt><dd className="font-medium">{item.location?.name ?? "Organisation"}</dd></div><div><dt className="text-slate-500">Expiry/review</dt><dd className="font-medium">{formatDate(item.reviewExpiryDate)}</dd></div></dl><p className="mt-4 text-xs font-semibold capitalize text-slate-500">{item.confidentiality.toLowerCase()}</p></Link>;
}
