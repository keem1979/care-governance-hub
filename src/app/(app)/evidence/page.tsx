...
export default async function EvidencePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-widest text-emerald-700">Document repository</p><h1 className="text-3xl font-bold">Evidence Library</h1><p className="mt-1 text-slate-600">Inspection-ready evidence, expiry dates and controlled versions.</p></div>{canUpload && <Link href="/evidence/new" className="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white">Upload evidence</Link>}</div>
