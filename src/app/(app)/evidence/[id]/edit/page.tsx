...
export default async function EditEvidencePage({ params }: { params: Promise<{ id:string }> }) {
  ...
  try {
    const [item,memberships,policies,providerTypes] = await Promise.all([
    ...
    return <main className="mx-auto max-w-4xl space-y-5"><div><Link href={`/evidence/${id}`} className="text-sm font-semibold text-emerald-700">← Back to evidence</Link><h1 className="mt-2 text-3xl font-bold">Edit evidence details</h1></div><EvidenceForm owners={memberships.map(({user})=>user)} locations={context.locations.map((location)=>({id:location.id,name:location.name}))} policies={policies} providerTypes={providerTypes} initial={{id:item.id,title:item.title,description:item.description??"",category:item.category,evidenceType:item.evidenceType,taxonomyFamilyKey:item.taxonomyFamilyKey??"",taxonomyTypeKey:item.taxonomyTypeKey??"",providerEvidenceTypeId:item.providerEvidenceTypeId??"",currentnessStatus:item.currentnessStatus??"CURRENT",ownerId:item.ownerId,locationId:item.locationId??"",evidenceDate:dateInput(item.evidenceDate),reviewExpiryDate:dateInput(item.reviewExpiryDate),tags:item.tags.join(", "),relatedModule:item.relatedModule??"",relatedRecordId:item.relatedRecordId??"",confidentiality:item.confidentiality,status:item.status,notes:item.notes??"",sourceType:item.sourceType,sourceName:item.sourceName??"",sourceReference:item.sourceReference??"",sourceUrl:item.sourceUrl??"",originalAuthor:item.originalAuthor??"",capturedAt:dateInput(item.capturedAt),provenanceNote:item.provenanceNote??""}} /></main>;
