...
export default async function PolicyCataloguePage() {
  ...
  return <main className="space-y-5"><Link href="/policies" className="inline-flex text-sm font-bold text-emerald-800">← Policy Library</Link><PolicyStudio brand={{ ...organisation, policyBrandName: organisation.name, policyLogoUploadedAt: organisation.policyLogoUploadedAt?.toISOString() ?? null }} canBrand={hasPermission(context.permissions, PERMISSIONS.ORGANISATION_MANAGE)} members={members.map(({ userId, user }) => ({ id: userId, name: user.name }))} templates={POLICY_CATALOGUE.map((template) => ({ key: template.key, title: template.title, category: template.category, cqcKey: template.cqcKey, purpose: template.purpose, sourceCount: sourceAnnex(template).length, exists: existingKeys.has(template.key) }))} /></main>;
}
