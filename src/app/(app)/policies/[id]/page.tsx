  ...
  return <main className="space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
      <div className="rounded-2xl border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold">Current document</h2>
        {generated ? <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><p className="font-bold text-emerald-950">Premium native policy</p><p className="mt-2 text-sm leading-6 text-emerald-900">This source-grounded draft can be read online, printed or saved as a branded PDF. It remains a draft until the organisation completes local review and approval.</p><Link href={`/policies/${id}/document`} className="mt-4 inline-flex rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">Open premium policy</Link></div> : currentUrl ? <><div className="mt-4 flex gap-2"><a href={currentUrl} target="_blank" className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">Open document</a><a href={`${currentUrl}?download=1`} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold">Download</a></div>{policy.currentVersion?.contentType === "application/pdf" && <iframe title="Policy document preview" src={currentUrl} className="mt-4 h-[520px] w-full rounded-xl border border-slate-200" />}</> : <p className="mt-3 text-slate-600">No current document.</p>}
      </div>
