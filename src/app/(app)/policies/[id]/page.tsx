...
export default async function PolicyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const policy = await db.policy.findFirst({
    ...
    include: { owner: { select: { name: true } }, approvedBy: { select: { name: true } }, evidenceRecord: { select: { id: true } }, currentVersion: true, versions: { include: { uploadedBy: { select: { name: true } } }, orderBy: { createdAt: "desc" } } },
  ...
  const generated = Array.isArray(policy.generatedSections) && policy.generatedSections.length > 0;
  return <main className="space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
      <div className="rounded-2xl border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold">Current document</h2>
        {generated ? <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><p className="font-bold text-emerald-950">ATOM licensed policy</p><p className="mt-2 text-sm leading-6 text-emerald-900">Open the organisation-branded policy, review its regulatory sources and prepare an individually watermarked print or PDF copy.</p><div className="mt-4 flex flex-wrap gap-2"><Link href={`/policies/${id}/document`} className="inline-flex rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">Open licensed policy</Link>{policy.evidenceRecord && <Link href={`/evidence/${policy.evidenceRecord.id}`} className="inline-flex rounded-lg border border-emerald-700 bg-white px-4 py-2 text-sm font-semibold text-emerald-800">View in Evidence Library</Link>}</div><p className="mt-3 text-xs font-medium text-emerald-800">The Evidence Library points to this controlled policy; no duplicate copy is stored.</p></div> : currentUrl ? <><div className="mt-4 flex gap-2"><a href={currentUrl} target="_blank" className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">Open document</a><a href={`${currentUrl}?download=1`} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold">Download</a></div>{policy.currentVersion?.contentType === "application/pdf" && <iframe title="Policy document preview" src={currentUrl} className="mt-4 h-[520px] w-full rounded-xl border border-slate-200" />}</> : <p className="mt-3 text-slate-600">No current document.</p>}
      </div>
    ...
    {canEdit && !generated && <section className="space-y-3"><h2 className="text-xl font-bold">Replace current version</h2><VersionUpload id={id} /></section>}
