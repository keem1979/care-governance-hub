...
export default async function PoliciesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="space-y-6">
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-2"><Link href={{ query: { ...params, view: "cards" } }} className={`rounded-lg px-3 py-2 text-sm ${view === "cards" ? "bg-emerald-100 font-semibold" : "bg-white"}`}>Cards</Link><Link href={{ query: { ...params, view: "table" } }} className={`rounded-lg px-3 py-2 text-sm ${view === "table" ? "bg-emerald-100 font-semibold" : "bg-white"}`}>Table</Link><Link href="/api/policies/export" className="rounded-lg bg-white px-3 py-2 text-sm">Export CSV</Link></div>
    </div>
