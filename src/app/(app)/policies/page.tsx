...
export default async function PoliciesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      {canEdit ? <Link href="/policies/new" className="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white">Add or upload policy</Link> : <span aria-disabled="true" className="cursor-not-allowed rounded-xl border border-slate-300 bg-slate-100 px-5 py-3 text-sm font-semibold text-slate-500">Add or upload policy</span>}
    </div>
...
function PolicyCard({ policy, canEdit }: { policy: { id: string; title: string; category: string; status: string; nextReviewDate: Date | null; owner: { name: string }; currentVersion: { versionNumber: string } | null; tags: string[] }; canEdit: boolean }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-400">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">{policy.category}</p><h2 className="mt-1 text-lg font-bold"><Link href={`/policies/${policy.id}`} className="hover:text-emerald-800 hover:underline">{policy.title}</Link></h2></div><Status value={display} /></div>
