...
export default async function AuditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return <main className="space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">Internal assurance score</p><p className="mt-1 text-3xl font-bold">{audit.overallScore === null ? "—" : `${audit.overallScore}%`}</p><p className="mt-1 text-xs text-slate-500">Not a CQC rating prediction.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">Open findings</p><p className="mt-1 text-3xl font-bold">{audit.findings.filter((item) => !item.resolvedAt).length}</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">Sign-off and evidence</p><p className="mt-2 font-bold">{audit.signedOffBy?.name ?? "Not signed off"}</p><p className="mt-1 text-xs text-slate-500">{generatedEvidence?"Evidence Library record active":"Created automatically on submission"}</p></div></section>
      ...
      {audit.findings.length > 0 ? <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-bold">Findings requiring attention</h2><p className="mt-1 text-sm text-amber-900">Partial and non-compliant checks remain visible until improvement is evidenced.</p></div><Link href={`/actions/new?sourceType=AUDIT&sourceId=${id}`} className="rounded-lg bg-amber-900 px-4 py-2 text-sm font-bold text-white">Create corrective action</Link></div><div className="mt-4 space-y-3">{audit.findings.map((finding) => <div key={finding.id} className="rounded-xl bg-white p-4"><div className="flex justify-between gap-3"><strong>{finding.summary}</strong><span className="rounded-full bg-red-100 px-2 py-1 text-xs font-bold text-red-800">{finding.severity}</span></div><p className="mt-1 text-sm text-slate-600">{finding.recommendation}</p>{finding.actionRequired ? <p className="mt-2 text-xs font-semibold text-amber-800">Corrective action required</p> : null}</div>)}</div></section> : null}
