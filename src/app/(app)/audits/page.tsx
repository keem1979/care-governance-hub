...
export default async function AuditsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  try {
    ...
    return <main className="space-y-7">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-bold">Audit Centre</h1><p className="mt-1 text-slate-600">Choose a form, check a sample, record what you found and submit the audit for sign-off.</p></div>{canComplete ? <Link href="/audits/new" className="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white">Start a new audit form</Link> : null}</div>
