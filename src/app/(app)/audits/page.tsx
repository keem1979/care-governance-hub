...
export default async function AuditsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  try {
    ...
    return <main className="space-y-7">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-700">Premium registered-manager assurance</p><h1 className="mt-1 text-3xl font-bold">Audit Centre</h1><p className="mt-1 text-slate-600">Choose an RM-grade form, define a defensible sample, record objective evidence and submit the audit for independent sign-off.</p></div>{canComplete ? <Link href="/audits/new" className="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white">Start a new audit form</Link> : null}</div>

      {canComplete ? <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="font-bold text-emerald-950">Complete the audit and keep the evidence together</h2><p className="mt-1 max-w-4xl text-sm leading-6 text-emerald-900">Choose a form, define the objective and sample, record what was checked, then submit it for review. Submitted audits are listed in the Evidence Library; unfinished drafts are not. Partial and failed checks create findings that can be assigned through the Action Tracker.</p></div><Link href="/audits/new" className="rounded-xl bg-emerald-800 px-4 py-2.5 text-sm font-semibold text-white">Choose an audit form</Link></div></section> : null}
      ...
      <section><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-bold">Choose an audit form</h2><p className="mt-1 text-sm text-slate-600">Forms cover care delivery, people’s experience, workforce, safety, clinical practice, information governance and leadership.</p></div><form><select name="category" defaultValue={category} className="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm"><option value="">All audit areas</option>{[...new Set(templates.map((item)=>item.category))].map((item)=><option key={item}>{item}</option>)}</select><button className="ml-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white">Show</button></form></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{shownTemplates.map((template) => {
