...
export default async function RegistersPage() {
  ...
  const [definitions, counts] = await Promise.all([
    db.registerDefinition.findMany({
      where: {
        isPublished: true,
        OR: [{ organisationId: null }, { organisationId: context.organisation.id }],
      },
      orderBy: { sortOrder: "asc" },
    }),
    db.registerEntry.groupBy({
      by: ["definitionId"],
      where: registerScopeWhere(context),
      _count: { _all: true },
    }),
  ]).finally(() => db.$disconnect());
  const countMap = new Map(
    counts.map((item) => [item.definitionId, item._count._all]),
  );
    ...
    <main className="space-y-7">
      <div>
        ...
        <p className="mt-1 text-slate-600">
          Consistent records, evidence, learning and follow-up across your service.
        </p>
      </div>
