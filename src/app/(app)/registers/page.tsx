...
export default async function RegistersPage({ searchParams }: { searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  return <main className="space-y-7">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-bold">Registers</h1><p className="mt-1 max-w-3xl text-slate-600">Record incidents, complaints, safeguarding concerns, medicines matters, workforce checks and other events that need follow-up.</p></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3 text-center"><p className="text-3xl font-bold text-emerald-950">{definitions.length}</p><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">available registers</p></div></header>
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><h2 className="font-bold text-emerald-950">Register records also appear as evidence</h2><p className="mt-1 text-sm leading-6 text-emerald-900">When you save an entry, the Evidence Library lists it against the matching evidence requirement. Make later changes in the register so everyone sees the same record.</p><Link href="/evidence" className="mt-3 inline-flex text-sm font-bold text-emerald-800">Open Evidence Library →</Link></section>
