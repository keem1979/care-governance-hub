...
export default async function NewRegisterEntryPage({ params, searchParams }: { params: Promise<{ key: string }>; searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  try {
    const [definition, memberships, evidence, clients, staff] = await Promise.all([
      ...
      db.client.findMany({where:{...clientScopeWhere(context),status:{in:["PROSPECT","ACTIVE","PAUSED"]}},select:{id:true,clientReference:true,firstName:true,lastName:true,preferredName:true},orderBy:[{lastName:"asc"},{firstName:"asc"}],take:500}),
      db.staffMember.findMany({where:{...workforceScopeWhere(context),employmentStatus:{not:"LEFT"}},select:{id:true,employeeReference:true,firstName:true,lastName:true,preferredName:true},orderBy:[{lastName:"asc"},{firstName:"asc"}],take:500}),
    ]);
