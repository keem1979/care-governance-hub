import { activityCsv, parseActivityFilters } from "@/lib/activity";
...
export async function GET(request: Request) {
  ...
  const where = {
    ...
    ...(filters.q ? { OR: [{ summary: { contains: filters.q, mode: "insensitive" as const } }, { recordType: { contains: filters.q, mode: "insensitive" as const } }, { recordId: { contains: filters.q, mode: "insensitive" as const } }, { user: { name: { contains: filters.q, mode: "insensitive" as const } } }] } : {}),
  };
  const db = createDb();
  try {
    ...
    await db.activityLog.create({ data: { organisationId: context.organisation.id, userId: context.user.id, action: "REPORT_GENERATION", recordType: "ActivityLog", summary: `Exported activity log (${rows.length} records).`, afterValue: { filters: { action: filters.action, recordType: filters.recordType, locationId: filters.locationId }, recordCount: rows.length } } });
