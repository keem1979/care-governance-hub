...
export async function POST(request: Request) {
  ...
  try {
    ...
    const required = ["kpi", "month", "actual"];
    if (!required.every((name) => header.includes(name))) throw new Error("CSV must include kpi, month and actual columns.");
    ...
    const records = rows.map((row, index) => {
      ...
      return { definition, locationId: location?.id ?? null, reportingMonth: parseKpiMonth(value("month")), actualValue, targetValue, greenThreshold, amberThreshold, ragStatus, notes: value("notes") || null };
    });
