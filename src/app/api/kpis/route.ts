...
export async function POST(request: Request) {
  ...
  try {
    ...
    const entry = await db.$transaction(async (tx) => {
      const data = { organisationId: context.organisation.id, locationId, kpiId, reportingMonth, actualValue, targetValue, greenThreshold, amberThreshold, ragStatus: ragStatus as never, notes, createdById: context.user.id };
      ...
      await tx.activityLog.create({ data: { organisationId: context.organisation.id, locationId, userId: context.user.id, action: existing ? "UPDATE" : "CREATE", recordType: "KpiEntry", recordId: saved.id, summary: `${existing ? "Updated" : "Recorded"} KPI: ${definition.name} for ${monthKey(reportingMonth)}`, afterValue: { actualValue, targetValue, greenThreshold, amberThreshold, ragStatus } } });
      return saved;
    });
