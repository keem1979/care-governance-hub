...
export async function GET(request: Request) {
  ...
  try {
    ...
    const header = ["KPI", "Month", "Location", "Actual", "Unit", "Target", "Green threshold", "Amber threshold", "RAG", "Evidence source", "Source link", "Notes", "Evidence attachments", "Recorded by"];
    const rows = entries.map((item) => [item.kpi.name, monthKey(item.reportingMonth), item.location?.name ?? "Organisation-wide", item.actualValue, item.kpi.unit, item.targetValue, item.greenThreshold, item.amberThreshold, item.ragStatus, kpiSourceLabel(item.sourceType), item.sourceUrl ?? "", item.notes ?? "", item._count.evidenceLinks, item.createdBy.name]);
