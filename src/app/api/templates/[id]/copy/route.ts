...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const evidence = await db.$transaction(async (tx) => {
      const created = await tx.evidence.create({ data: { organisationId: context.organisation.id, locationId, title: `${template.title} - working copy`, description: `Branded working copy from Template Library: ${template.description}`, category: templateEvidenceCategory(template.category), evidenceType: "Form", ownerId, tags: [...template.tags, "template-copy", "organisation-branded"], relatedModule: "Template", relatedRecordId: template.id, confidentiality: "INTERNAL", notes: `Based on ${template.title} version ${template.version}. Organisation branding was applied automatically.`, uploadedById: context.user.id } });
