...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await db.$transaction(async(tx)=>{
      ...
      await tx.carePlanStaffAssignment.deleteMany({where:{carePlanId:id,versionId:version.id}});
      if(staffIds.length)await tx.carePlanStaffAssignment.createMany({data:staffIds.map((staffMemberId)=>({carePlanId:id,versionId:version.id,staffMemberId}))});
      await tx.carePlan.update({where:{id},data:{locationId:data.locationId,clientId:data.clientId!,status:"AWAITING_APPROVAL",overallRisk:data.overallRisk as never,nextReviewDate:date(data.nextReviewDate),careCoordinatorId:data.careCoordinatorId,registeredManagerId:data.registeredManagerId,serviceType:data.serviceType,fundingType:data.fundingType,localAuthorityCode:data.localAuthorityCode,localAuthorityName:data.localAuthorityName,commissioner:data.commissioner,linkedActionIds}});
