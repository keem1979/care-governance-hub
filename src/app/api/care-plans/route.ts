...
export async function POST(request: Request) {
  ...
  try {
    ...
    const plan = await db.$transaction(async (tx) => {
      const created = await tx.carePlan.create({ data: { organisationId: context.organisation.id, locationId: data.locationId, clientId: data.clientId!, reference, currentVersionNumber: 0, status: data.status as never, overallRisk: data.overallRisk as never, nextReviewDate: date(data.nextReviewDate), careCoordinatorId: data.careCoordinatorId, registeredManagerId: data.registeredManagerId, serviceType: data.serviceType, fundingType: data.fundingType, localAuthorityCode: data.localAuthorityCode, localAuthorityName: data.localAuthorityName, commissioner: data.commissioner } });
      ...
      await tx.carePlan.update({ where: { id: created.id }, data: { linkedActionIds, status: linkedActionIds.length ? "AWAITING_APPROVAL" : data.status as never, assignments: { create: staffIds.map((staffMemberId) => ({ staffMemberId, versionId: version.id })) } } });
