...
export async function GET() {
  const context = await requireAnyPermission([PERMISSIONS.GOVERNANCE_VIEW, PERMISSIONS.REPORTS_EXPORT, PERMISSIONS.ORGANISATION_MANAGE]);
  const db = createDb();
  try {
...
export async function POST(request: Request) {
  ...
  const storageKey = `${context.organisation.id}/company-logo/${crypto.randomUUID()}`;
  const db = createDb();
  try {
    ...
    try {
      await db.$transaction([
        ...
        db.activityLog.create({ data: { organisationId: context.organisation.id, userId: context.user.id, action: "UPDATE", recordType: "Organisation", recordId: context.organisation.id, summary: "Updated the company logo used on generated documents.", beforeValue: { fileName: current?.policyLogoFileName }, afterValue: { fileName: logo.name, contentType: logo.type, size: logo.size } } }),
      ]);
    ...
    return NextResponse.json({ message: "Company logo uploaded and applied to generated documents." });
  } catch (error) {
...
export async function DELETE() {
  ...
  try {
    ...
    await db.$transaction([
      ...
      db.activityLog.create({ data: { organisationId: context.organisation.id, userId: context.user.id, action: "UPDATE", recordType: "Organisation", recordId: context.organisation.id, summary: "Removed the company logo from generated documents.", beforeValue: { fileName: current.policyLogoFileName } } }),
    ]);
    await deletePolicyFile(current.policyLogoStorageKey);
    return NextResponse.json({ message: "Company logo removed from generated documents." });
  } finally { await db.$disconnect(); }
}
