  ...
  try {
    ...
    try {
      ...
      for(const item of submitted) {
        ...
        if(item.answer==="NON_COMPLIANT") await db.auditFinding.upsert({where:{responseId:response.id},create:{auditId:id,responseId:response.id,severity:"HIGH",summary:question.text,recommendation:item.comment.trim()||"Create and complete a corrective action.",actionRequired:true},update:{summary:question.text,recommendation:item.comment.trim()||"Create and complete a corrective action.",actionRequired:true,resolvedAt:null}});
      ...
      await db.$transaction([
        db.audit.update({where:{id},data:{status,overallScore,strengths:body.strengths?.trim()||null,risks:body.risks?.trim()||null,recommendations:body.recommendations?.trim()||null,reviewDate:body.reviewDate?new Date(`${body.reviewDate}T12:00:00.000Z`):null}}),
        db.activityLog.create({data:{organisationId:context.organisation.id,locationId:audit.locationId,userId:context.user.id,action:"UPDATE",recordType:"Audit",recordId:id,summary:`${body.intent==="submit"?"Submitted":"Saved"} audit: ${audit.title}`,afterValue:{status,overallScore}}}),
      ]);
