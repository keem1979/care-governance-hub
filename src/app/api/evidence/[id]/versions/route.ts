...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    try {
      ...
      if (["RegisterEntry","Audit","Risk"].includes(evidence.relatedModule??"")) return NextResponse.json({ error: "This evidence is updated automatically from its source record. Add supporting evidence to the source workflow instead." }, { status: 409 });
